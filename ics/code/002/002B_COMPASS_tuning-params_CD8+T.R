...
for (stim in stims)
{
  stimdir <- glue("/fh/fast/mcelrath_j/vvoillet_working/CHIL/Projects/BCG_TICE/ICS/output/002_output/tuning/{stim}")
  dir.create(stimdir, recursive = TRUE, showWarnings = FALSE)
  trt_stim <- stim

  # Tune the parameters - output are saved to <stimdir>
  tuning_results <- tune_parameters(cc8,
                                    trt_stim,
                                    ctrl_stim,
                                    min_count = 9,
                                    min_cells = 4,
  ...
  write_rds(x = tuning_results$params[[length(tuning_results$params)]], file = glue("/fh/fast/mcelrath_j/vvoillet_working/CHIL/Projects/BCG_TICE/ICS/output/002_output/tuning/tuned_params_CD8+T_{stim}.rds"))
}
