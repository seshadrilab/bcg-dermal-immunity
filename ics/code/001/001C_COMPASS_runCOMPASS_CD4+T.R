...
for(trt_stim in stims){
  # Parameter
  params <- read_rds(glue("/fh/fast/mcelrath_j/vvoillet_working/CHIL/Projects/BCG_TICE/ICS/output/001_output/tuning/tuned_params_CD4+T_{trt_stim}.rds"))
  ...
  for(i in 1:10){
    ...
    # Messages
    message(glue("CD4 {trt_stim} COMPASS run"))
    message(glue("control: {ctrl_stim}"))
    message(glue("{n_iter} iterations, {n_rep} replications, seed {s}"))

    # Output files
    outfolder <- glue("/fh/fast/mcelrath_j/vvoillet_working/CHIL/Projects/BCG_TICE/ICS/output/001_output/tuning/{trt_stim}")
    outfile <- glue("CD4+T_{ctrl_stim}_{trt_stim}_tuned_run_{n_iter}iter_{n_rep}rep_seed{s}.rds")
    outfilepattern <- stringr::str_replace_all(outfile, "\\+", "\\\\+")
    message(glue("Checking for file {outfile} in {outfolder}"))

    # COMPASS
    set.seed(s)
    run_results <- COMPASS(cc4,
