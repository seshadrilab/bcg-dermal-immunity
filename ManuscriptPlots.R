...
# CyTOF NRS
cytof_nrs <- readRDS("/home/emmabishop/workspace/bcg_challenge_manuscript_inh/nrs_plt.rds")

# One of the CyTOF FlowJo workspaces
wsfile <- "/home/emmabishop/workspace/human-bcg-challenge-cytof/cytof/data/20221228_fcs_files/20221230_BCG Skin Biopsy 2.wsp"
fcs_dir <- "/home/emmabishop/workspace/human-bcg-challenge-cytof/cytof/data/20221228_fcs_files/20221230 BCG Skin Biopsy 2 Live CD45+ cells FCS"
...
#################################
## CyTOF cell type frequencies ##
#################################

# Inspired by cytof/scripts/02_DA_and_Fxn.R and cytof/scripts/Helper_Functions.R

all_count <- all_full_count %>%
  dplyr::filter(!cluster_id %in% c("BCG12_1", "BCG07_1", "BCG07_2")) %>% # drop BCG12 because no Day 15 timepoint)
  pivot_longer(cols = 7:20, names_to = "cluster", values_to = "freq") %>%
...
ggsave(file.path(outdir, "Fig4_cytof_line.png"), plot = pgrid,
       dpi = 300, width = 7.75, height = 3.75, device = "png")

cairo_pdf(file = file.path(outdir, "Fig4_cytof_line.pdf"), 
          width = 7.75, height = 3.75, bg = "transparent", family = "Arial")
print(pgrid)
dev.off()

# Adjusted p-values
all_plots_adj <- purrr::pmap(.l = list(clusters),
                         .f = function(n) {
                           make_mag_plots(all_count, current_cluster = n, adjust_p = TRUE)
                         })

names(all_plots_adj) <- clusters
pgrid_adj <- ggarrange(plotlist = all_plots_adj, nrow = 3, ncol = 6)
pgrid_adj <- annotate_figure(pgrid_adj, left = text_grob("Frequency (% CD45+)", rot = 90, size = 9))
pgrid_adj
...
cytof_qc_nrs <- cytof_nrs +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 10)) +
  scale_color_manual(labels = c("Day 3", "Day 15"), values = c("#0571B0", "#E66101")) +
  theme_classic() +
  theme(text = element_text(family="Arial"),
...
############################
## CyTOF gating tree info ##
############################
# So I can pick representative gates. 
# CONCLUSION: Data was gated up to CD45+ then all other annotation done in R.
# Most of these gates were not used (for example, no workspace imported into R).
