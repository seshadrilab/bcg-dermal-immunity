...
At first I used HTODemux for Day 3 and MULTIseqDemux for Day 15 because the ridge plots had better separation at Day 3 that way. However, we get 1664 more singlest with MULTIseqDemux at Day 3 and the hashtag assignment is almost entirely in agreement between the two tools. So I've decided to use MULTIseqDemux for both timepoint for consistency.
...
if(!dir.exists(file.path(script_output_dir))) {
  ...
  cat(sprintf("Creating folder %s\n", file.path(script_output_dir, "out")))
  dir.create(file.path(script_output_dir, "out"), recursive = T)
}
```
