...
pop_levels_d3 <- c("Monocytes", "Myeloid other", "B cells", "T cells", "Lymphoid other", "Tcm", "MAIT cells")
pop_levels_d15 <- c("Monocytes", "Myeloid other", "B cells", "T cells", "Lymphoid other", "Tcm", "MAIT cells", "Granulocytes")

day3_sub@active.ident <- factor(x = day3_sub@active.ident, levels = pop_levels_d3)
day15_sub@active.ident <- factor(x = day15_sub@active.ident, levels = pop_levels_d15)
...
Do a two-sided Wilcoxon rank-sum test on every possible combination of cell types, followed by Bonferroni correction and filtering results by those with adjusted p-value < 0.0001.

```{r}
# Module1
test_combos_mod1 <- function(celltypes, obj) {
  combos <- combn(celltypes, 2, simplify = FALSE)  # Get unique pairs of cell types
  
  big_df <- NULL
  for (combo in combos) {
    testdat <- obj@meta.data %>% filter(cytof_annot %in% combo)
    wilcox_p <- wilcox.test(Module1 ~ cytof_annot, 
                               data = testdat,
                               alternative = c("two.sided"),
                               exact = FALSE)$p.value
    temp_df <- data.frame(combo = paste(combo, collapse = "_"), 
                          padj = p.adjust(wilcox_p, method = "bonferroni"))
    big_df <- bind_rows(big_df, temp_df)
  }
  out <- big_df %>%
    filter(padj < 0.0001)
  return(out)
...
d3_mod1 <- test_combos_mod1(pop_levels_d3, day3_sub_t)
d15_mod1 <- test_combos_mod1(pop_levels_d15, day15_sub_t)


# Module 2
test_combos_mod2 <- function(celltypes, obj) {
  combos <- combn(celltypes, 2, simplify = FALSE)  # Get unique pairs of cell types
  
  big_df <- NULL
  for (combo in combos) {
    testdat <- obj@meta.data %>% filter(cytof_annot %in% combo)
    wilcox_p <- wilcox.test(Module2 ~ cytof_annot, 
                               data = testdat,
                               alternative = c("two.sided"),
                               exact = FALSE)$p.value
    temp_df <- data.frame(combo = paste(combo, collapse = "_"), 
...
d3_mod2 <- test_combos_mod2(pop_levels_d3, day3_sub_t)
d15_mod2 <- test_combos_mod2(pop_levels_d15, day15_sub_t)
```
...
# Generate all unique pairs of cell types
combinations <- combn(pop_levels_d3, 2, simplify = FALSE)

pval_list_mod1_d3 <- NULL

for (c in combinations) {
  testdat <- day3_sub_t@meta.data %>%
    filter(cytof_annot %in% c)

  wilcox_test <- wilcox.test(Module1 ~ cytof_annot, data = testdat,
                   alternative = c("two.sided"),
                   exact = FALSE)$p.value
  
  temp_df <- data.frame(combo = paste(c, collapse = "_"), pval = p.adjust(wilcox_test, method = "BH"))
  
  pval_list_mod1_d3 <- bind_rows(pval_list_mod1_d3, temp_df)
}

# Which are significant by the strict 
pval_list_mod1_d3 %>%
  filter(pval < 0.0001)
...
# Neutrophil Day 3
VlnPlot(day3_sub_t, "Module1", pt.size = 0.1, cols = pop_colors) + 
  NoLegend() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size = 9),
        axis.text.y = element_text(size = 9),
        plot.title = element_text(hjust = 0.5, face = "plain", size = 9)) +
  ggtitle("Neutrophils (I, II)")


# Use two-sided Wilcoxon rank-sum tests followed by Bonferroni correction
# Test every combo of cell populations
# ****adj. p-value < 0.0001


testdat2 <- testdat %>%
  filter(cytof_annot %in% c("T cells", "MAIT cells"))

wilcox_test <- wilcox.test(Module1 ~ cytof_annot, data = testdat2,
                   alternative = c("two.sided"),
                   exact = FALSE)

p.adjust(wilcox_test$p.value, method = "BH")
...
testdat <- day3_sub_t@meta.data %>%
  group_by(orig.ident, PTID, cytof_annot) %>%
  mutate(mod1_means = mean(Module1)) %>%
  ungroup() %>%
  select(orig.ident, PTID, cytof_annot, mod1_means, Module1) %>%
  distinct()

# Neutrophil Day 3
ggboxplot(testdat, x = "cytof_annot", y = "Module1", 
          color = "cytof_annot", palette = pop_colors,
          ylab = "mod1_means", xlab = "cytof_annot")
...
# Idents(day3_sub_t) <- "cytof_annot"
# Idents(day15_sub_t) <- "cytof_annot"
...
```{r}
combined <- merge(day3_sub_t, day15_sub_t)

combined@meta.data <- combined@meta.data %>%
  mutate(orig.ident = factor(orig.ident, levels = c("Day3", "Day15")))

saveRDS(combined, file = file.path(script_output_dir, "processed_data/11_combined_modules.rds"))
```

Violin plots

```{r}
neutr_vln <- VlnPlot(combined, "Module1", split.by = "orig.ident", 
        cols = c("Day3" = "white", "Day15" = "gray50"), pt.size = 0) + 
  ylim(-0.17, 0.52) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size = 9),
        axis.text.y = element_text(size = 9),
...
mono_vln <- VlnPlot(combined, "Module2", split.by = "orig.ident", 
        cols = c("Day3" = "white", "Day15" = "gray50"), pt.size = 0) + 
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size = 9),
        axis.text.y = element_text(size = 9),
        ...
        legend.key.size = unit(0.3, "cm"),
        plot.title = element_text(hjust = 0.5, face = "plain", size = 9)) +
  ggtitle("Monocytes (II, IV)")
mono_vln


grid_vln <- ggarrange(plotlist = list(neutr_vln, mono_vln), nrow = 2, ncol = 1) %>%
  annotate_figure(left = text_grob("Module Score", rot = 90, size = 9))
grid_vln

# Save
ggsave(file.path(script_output_dir, "plots/Fig5_violin.png"), plot = grid_vln,
       dpi = 300, width = 3.5, height = 5, device = "png")

cairo_pdf(file = file.path(script_output_dir, "plots/Fig5_violin.pdf"), 
          width = 3.5, height = 5, bg = "transparent", family = "Arial")
print(grid_vln)
dev.off()
```
